...
begin
    if reset = '1' then
        ...
        -- Slow down character cycling
        if letter_counter = "10111110101111000010000" then  -- Adjust for timing
            letter_counter <= (others => '0');

            if curr_state = UNLOCKED then
                case char_index is
                    when 0 => dial_num <= "1010";  -- O
                    when 1 => dial_num <= "1011";  -- P
                    when 2 => dial_num <= "1100";  -- E
                    when 3 => dial_num <= "1101";  -- N
                    when others => dial_num <= "0000";
                end case;
            ...
            elsif curr_state = ERROR then
                case char_index is
                    when 0 => dial_num <= "1110";  -- E
                    when 1 => dial_num <= "1111";  -- R
                    when 2 => dial_num <= "1111";  -- R again
                    when others => dial_num <= "0000";
                end case;
...
begin
    case curr_state is
        ...
        when UNLOCKED =>
            null; --Display is handled in spearate clock process
            -- TODO
        when COMPARING =>
